	...
	class WC_Noqodi_Payment_Gateway extends WC_Payment_Gateway {
		...
		public function process_payment( $order_id)
		{
         ...
         foreach ($response_json as $key => $item) {
            foreach($item as $key => $infoDetail){
                if($key == 'status'){
                    $noqodi_preauth_response_status =$infoDetail;
                }
                elseif($key == 'errorCode')
                {
                    foreach($infoDetail as $key => $statusInfoErrorDetailVal)
                    {
                        if($key == 'message'){
                        $noqodi_error_msg =$statusInfoErrorDetailVal;
                        }
                    }
                }
                if($key == 'preAuthToken'){
                  $preAuthToken =$infoDetail;
             }
           }
         }
        ...
        function generateAccessToken($method, $url){
                   ...
                   curl_close($curl);
                   $access_token_resposne = json_decode($result, true);
                   $errors = $access_token_resposne['response']['errors'];
                   $data = $access_token_resposne['response']['data'][0];
                  foreach ($access_token_resposne as $key => $infoDetail) {
                        if($key == 'statusInfo'){
                             foreach($infoDetail as $key => $statusInfoDetail){
                               if($key == 'status'){
                                  $access_token_resposne_status =$statusInfoDetail;
                                            }
                                    elseif($key == 'errorCode')
                                    {
                                            foreach($statusInfoDetail as $key => $statusInfoErrorDetailVal)
                                            {
                                                if($key == 'message'){
                                                  $noqodi_error_msg =$statusInfoErrorDetailVal;
                                                 }
             ...
             $noqodi_capture_response_status ='';
          	  ...
          	  foreach ($final_noqodi_auth_response as $key => $item) {
                   foreach($item as $key => $infoDetail){
                     ...
                     foreach($infoDetail as $key => $statusInfoDetail){
                            if($key == 'status'){
                                  $noqodi_auth_response_status =$statusInfoDetail;
                                    }
                            elseif($key == 'errorCode')
                            {
                                    foreach($statusInfoDetail as $key => $statusInfoErrorDetailVal)
                       ...
                       if($key == 'merchantInfo')
                       {
                        foreach($infoDetail as $key => $merchantInfoDetail){
                          if($key == 'merchantOrderId'){
                            $merchantOrderId=$merchantInfoDetail;
                          }
                        }
                       }
                        if($key == 'paymentInfo')
                        {
                          foreach($infoDetail as $key => $paymentInfoDetail){
                            if($key == 'noqodiOrderId'){
                               $noqodiOrderId=$paymentInfoDetail;
                            }
                          }
                        }
                      }
           ...
           if($this->is_auto_capture_enabled == 'yes')
            {
             ...
             foreach ($noqodi_capture_response as $key => $infoDetail) {
                            if($key == 'statusInfo'){
                               foreach($infoDetail as $key => $statusInfoDetail){
                                   if($key == 'status'){
                                        $noqodi_capture_response_status =$statusInfoDetail;
                                   }
                                   elseif($key == 'errorCode')
                                   {
                                     foreach($statusInfoDetail as $key => $statusInfoErrorDetailVal)
