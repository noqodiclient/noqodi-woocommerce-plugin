...
if ( $wp_customize->changeset_post_id() ) {
	...
	if ( $missed_schedule ) {
		...
		wp_die(
			...
			'<p><a href="' . esc_url( remove_query_arg( 'changeset_' ) ) . '">' . __( 'Customize New Changes' ) . '</a></p>' . $script,
			200
		);
	}
	...
	if ( in_array( get_post_status( $changeset_post->ID ), array( 'publish', 'trash' ), true ) ) {
		wp_die(
			...
			'<p><a href="' . esc_url( remove_query_arg( 'changeset_' ) ) . '">' . __( 'Customize New Changes' ) . '</a></p>',
			403
		);
	}
}
